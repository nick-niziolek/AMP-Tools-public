    ...
    if (nLinks() == 2) {
        ...
        for (int iter = 0; iter < 5; iter++) {
            ...
            for (int j = 0; j < nLinks(); j++) {
                joint_angles(j) = amp::RNG::randf(0.0, 2 * M_PI); //
            }
            // Re-initialize joint positions based on new joint angles
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                for (int j = nLinks()-1; j >= 0; j--) {
                    ...
                    if (direction.norm() < 0.9 || direction.norm() > 1.1) {
                        std::cout << "Direction norm in backward pass incorrect, breaking: " << direction.norm() << std::endl;
                        break;
                    }
                ...
                for (int j = 0; j < nLinks(); j++) {
                    ...
                    if (direction.norm() < 0.9 || direction.norm() > 1.1) {
                        std::cout << "Direction norm in forward pass incorrect, breaking: " << direction.norm() << std::endl;
                        break;
                    }
                ...
                if (direction.norm() < 0.9 || direction.norm() > 1.1) {
                    std::cout << "Direction norm too small, breaking out of iterations." << std::endl;
                    break;
                }
                ...
                if ((joint_positions[nLinks()] - end_effector_location).norm() < tol) {
                    ...
                    for (int j = 0; j < nLinks(); j++) {
                        ...
                        double angle = atan2(vec.y(), vec.x()) - atan2(ref.y(), ref.x());
                        // Normalize angle to [0, 2*pi)
                        angle = fmod(angle + 2 * M_PI, 2 * M_PI);
                        joint_angles(j) = angle;
                    }
                    std::cout << "Converged in " << i << " iterations with error: "
                              << (joint_positions[nLinks()] - end_effector_location).norm()
                              << ", last joint position: ["
                              << joint_positions[nLinks()].x() << ", "
